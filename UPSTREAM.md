...
- **Workflow:** useful frozen-install, lint, clean-diff and build checks may be absorbed without changing the local-only customer installer policy.
...
5. Preserve Plus identity, managed settings, per-node Ping binding, data semantics, fallbacks, privacy boundaries and local-only installer policy.
